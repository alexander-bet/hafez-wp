...
function optionsframework_fields()
{
	...
	foreach ($options as $value) {
		...
		if (($value['type'] != "heading") && ($value['type'] != "info")) {
			...
			if (isset($value['name'])) {
				...
				if (isset($value['tutorial'])) {
					$output .= '<h4 class="heading">' . esc_html($value['name']) . ' <a href="' . esc_html($value['tutorial']) . '" target="_blank" style=" color:#cc181e; text-decoration:none; float:right">' . esc_html__('Video Tutorial', 'hafezkids') . ' &nbsp;<i class="fa fa-youtube-play" style="font-size:1.2em;" aria-hidden="true"></i></a></h4>' . "\n";
				} else {
		...
		switch ($value['type']) {
			...
			case 'typography':
				...
				if ($typography_options['sizes']) {
					$font_size = '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Font Size:', 'hafezkids') . '</div><select class="of-typography of-typography-size" name="' . esc_attr($option_name . '[' . $value['id'] . '][size]') . '" id="' . esc_attr($value['id'] . '_size') . '">';
				...
				if ($typography_options['faces']) {
					$font_face = '<div class="half-box cf"><div class="hafez_setting_title">' . esc_html__('The Font:', 'hafezkids') . '</div><select class="of-typography of-typography-face" name="' . esc_attr($option_name . '[' . $value['id'] . '][face]') . '" id="' . esc_attr($value['id'] . '_face') . '">';
				...
				if ($typography_options['styles']) {
					$font_style = '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Font Style:', 'hafezkids') . '</div><select class="of-typography of-typography-style" name="' . esc_attr($option_name) . '[' . esc_attr($value['id']) . '][style]" id="' . esc_attr($value['id']) . '_style">';
					$styles = $typography_options['styles'];
					foreach ($styles as $key => $style) {
						$font_style .= '<option value="' . esc_attr($key) . '" ' . selected($typography_stored['style'], $key, false) . '>' . esc_html($style) . '</option>';
					}
				...
				if ($typography_options['transform']) {
					$font_transform = '<div class="half-box cf"><div class="hafez_setting_title">' . esc_html__('Font Transform:', 'hafezkids') . '</div><select class="of-typography of-typography-transform" name="' . esc_attr($option_name) . '[' . esc_attr($value['id']) . '][transform]" id="' . esc_attr($value['id']) . '_transform">';
					$transforms = $typography_options['transform'];
					foreach ($transforms as $key => $transform) {
						$font_transform .= '<option value="' . esc_attr($key) . '" ' . selected($typography_stored['transform'], $key, false) . '>' . esc_html($transform) . '</option>';
					}
				...
				if ($typography_options['weight']) {
					$font_weight = '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Font Weight:', 'hafezkids') . '</div><select class="of-typography of-typography-weight" name="' . esc_attr($option_name) . '[' . esc_attr($value['id']) . '][weight]" id="' . esc_attr($value['id']) . '_weight">';
					$weights = $typography_options['weight'];
					foreach ($weights as $key => $weight) {
						$font_weight .= '<option value="' . esc_attr($key) . '" ' . selected($typography_stored['weight'], $key, false) . '>' . esc_html($weight) . '</option>';
					}
				...
				if ($typography_options['lineheight']) {
					$font_lineheight = '<div class="half-box cf"><div class="hafez_setting_title">' . esc_html__(
				...
				if ($typography_options['color']) {
					$font_color = '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Font Color:', 'hafezkids') . '</div><div id="' . esc_attr($value['id']) . '_color_picker" class="colorSelector"><div style="' . esc_attr('background-color:' . $typography_stored['color']) . '"></div></div>';
			...
			case 'header_styles':
				...
				// Header BG Color
				$output .= '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Header Background Color:', 'hafezkids') . '</div>';
				...
				// Text Color
				$output .= '<div class="half-box cf"><div class="hafez_setting_title">' . esc_html__('Text Color:', 'hafezkids') . '</div>';
				...
				// Background Image - New AJAX Uploader using Media Library
				$output .= '<div class="hafez_setting_line cf"><div class="hafez_setting_title">' . esc_html__('Header Background Image:', 'hafezkids') . '</div> ';
				...
				// Header Width
				$output .= '<div class="hafez_setting_line cf"><div class="half-box"><div class="hafez_setting_title">' . esc_html__('Width (px):', 'hafezkids') . '</div>';
				...
				// Header Height
				$output .= '<div class="half-box cf"><div class="hafez_setting_title">' . esc_html__('Height (px):', 'hafezkids') . '</div>';
				...
				// Content Alignment
				$output .= '<div class="hafez_setting_line cf"><div class="hafez_setting_title">' . esc_html__('Content Align:', 'hafezkids') . '</div>';
		...
		$hafez_dependency_script .= $field_script;
	}

	//Print dependency fields minified script
	// make it into one long line
	$hafez_dependency_script = str_replace(array("\n", "\r"), '', $hafez_dependency_script);
	// replace all multiple spaces by one space
	$hafez_dependency_script = preg_replace('!\s+!', ' ', $hafez_dependency_script);
	// replace some unneeded spaces, modify as needed
	$hafez_dependency_script = str_replace(array(' {', ' }', '{ ', '; '), array('{', '}', '{', ';'), $hafez_dependency_script);


	echo '<script>jQuery(document).ready(function($) {' . $hafez_dependency_script . '});	</script>';
